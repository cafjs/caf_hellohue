...
const VR_SESSION = 'vr'; // VR view
const AR_SESSION = 'ar'; // AR view
...
const SUPPORTED_DEVICES = {PHILIPS_HUE, MAGIC_LIGHT, TRIONES};
...
const notifyWebApp = function(self, msg) {
    ...
    (mySession !== VR_SESSION) &&
        self.$.session.notify([state], VR_SESSION);
    (mySession !== AR_SESSION) &&
        self.$.session.notify([state], AR_SESSION);
};
...
exports.methods = {
    ...
    async __ca_init__() {
        ...
        this.$.session.limitQueue(1, VR_SESSION); // ditto
        this.$.session.limitQueue(1, AR_SESSION); // ditto
        ...
        Object.keys(SUPPORTED_DEVICES).forEach(x => {
            ...
            this.state.config[x] = {
                ...
                /* Filtering by a name prefix is needed when services are not
                   advertised, as is the case for some Triones bulbs. If
                   the name prefix is not null we assume that is the case, and
                   we no longer filter by service uuid...*/
