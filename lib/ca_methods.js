...
const notifyWebApp = function(self, msg) {
    const state = myUtils.deepClone(self.state, x => CLEANUP_KEYS.includes(x));
...
exports.methods = {
    ...
    async __ca_init__() {
        ...
        // example config
        this.state.config = {
            serviceDiscover: stripDashes(this.$.props.serviceDiscover),
            serviceControl: stripDashes(this.$.props.serviceControl),
            charLight: stripDashes(this.$.props.charLight),
            charBrightness: stripDashes(this.$.props.charBrightness),
            charColor: stripDashes(this.$.props.charColor)
        };
    ...
    async switchLight(isOn) {
        ...
        doBundle(this, 'switchLight', isOn);
    ...
    async setBrightness(level) {
        ...
        doBundle(this, 'setBrightness', level);
        notifyWebApp(this, 'New inputs');
        return this.getState();
    },
    ...
    async setColorTemperature(level) {
        ...
        this.state.isColor = false;
        doBundle(this, 'setColorTemperature', level);
        notifyWebApp(this, 'New inputs');
        return this.getState();
    },
